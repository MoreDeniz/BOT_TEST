...
def create_kb_event_navigation(data: dict, join_button: str, user_id: int):
    ...
    if data.get('date'):
        ...
        prev_menu = 'date'
    if data.get('org_id'):
        events_list = db.get_all_events_by(date=data.get('org_id'))
        prev_menu = 'org'
    ...
    if current_id == 0:
        ...
        btn_join = InlineKeyboardButton(text=join_button, callback_data=select_event.new(
                                         menu='ev',
    ...
    if data.get('city') and not data.get('location'):
        ...
        btn_back = InlineKeyboardButton(text='Назад',
                                        callback_data=select_event.new(menu=prev_menu,
                                                                       location='',
                                                                       city=data.get('city', ''),
                                                                       org_id=data.get('org_id', 0),
                                                                       date=data.get('date', ''),
    ...
    btn_prev = InlineKeyboardButton(text='<<<',
                                    callback_data=select_event.new(menu='ev',
                                                                   location=data.get('location', ''),
                                                                   city=data.get('city', ''),
                                                                   org_id=data.get('org_id', 0),
                                                                   date=data.get('date', ''),
