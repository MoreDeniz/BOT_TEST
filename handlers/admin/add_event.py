...
from keyboards.standart import kb_yes_no, kb_city_select, create_kb_loca_select, kb_cancel


@dp.message_handler(commands=['add_event', 'новая туса'], state=None)
async def add_new_event(message: Message, user_status: str):
    if user_status == 'Админ':
        ...
        await message.answer(f'Извините, вам эта команда недоступна')
...
# Предложил нам города

# Ждём - получаем город, для него создаются клавиатурки:
...
# Предложил нам площадки

# Ждём - получаем площадку:
# Достаём город, который мы записали, если есть переходим к дате:
...
async def enter_name_loca(message: Message, state: FSMContext):
    ...
    if find:
        ...
        await message.answer(f'Такой площадки нет в нашей БД. Давай её зарегистрируем. '
...
async def enter_confirm_loca(message: Message, state: FSMContext):
    if message.text in ['Да', 'Нет']:  # Всё собираем и в БД
        if message.text == 'Да':
            data = await state.get_data()
            location = {"city": data["city_loca"], "name": data["name_loca"],
                     "address": data["address_loca"],
                     "phone": data["phone_loca"], "url": data["url_loca"]}
...
async def enter_url(message: Message, state: FSMContext):
    ...
    message_text = f'Название: {data["name"]}\nОписание: {data["description"]}\n' \
                   ...
                   f'Дата: {data["date"]}\nЦена: {data["price"]}\n\nДанные введены верно?'
    await db.bot.send_photo(message.from_user.id, photo=data['poster'],
                            caption=message_text, reply_markup=kb_yes_no)
