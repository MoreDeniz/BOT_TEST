...
async def start_message(message: Message, user_status: str):
    if user_status == 'Admin':
        ...
        await message.answer(f'Приветствую, {message.from_user.first_name}!\n'
                             f'Твой id в базе не найден. Давай его создадим\n '
                             f'Введи свои Имя и Фамилию:', reply_markup=kb_cancel)
        await User.name.set()
...
async def enter_role(message: Message, state: FSMContext):
    if message.text == 'Организатор':
        ...
        await message.answer('Выберите роль: Организатор, Акро или Гость', reply_markup=kb_user_role)
...
async def enter_code(message: Message, state: FSMContext):
    if message.text in admin_promo_code:
        ...
        await User.next()
    elif message.text in admin_promo_code:
        await state.update_data({'role': 'Acro'})
        await message.answer(f'Поздравляем! Твой статус Акро!\nВведи свой телефон:', reply_markup=kb_cancel)
        await User.next()
    else:
