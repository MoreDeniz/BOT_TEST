...
async def show_current_event(call: CallbackQuery):
    ...
    acros_id_list = list(map(lambda x: x[0], db.get_acros(data.get('event_id'))))
    join_button = 'Выписаться' if call.from_user.id in acros_id_list else 'Записаться'
    acros_id_list = [db.get_user_name(user_id)[0] for user_id in acros_id_list]

    acros = '\n'.join([f'{i}. {acro}' for i, acro in enumerate(acros_id_list, 1)])
